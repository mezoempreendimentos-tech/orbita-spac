...
export const demands = mysqlTable("demands", {
  ...
  budgetRubricCode: varchar("budgetRubricCode", { length: 32 }),
  budgetAcknowledgedAt: timestamp("budgetAcknowledgedAt"),
  budgetAcknowledgedByUserId: int("budgetAcknowledgedByUserId").references(() => users.id),
  budgetNote: text("budgetNote"),
...
export const demandCaseEvents = mysqlTable("demand_case_events", {
  id: int("id").autoincrement().primaryKey(),
  demandId: int("demandId").notNull(),
  eventType: mysqlEnum("eventType", ["analysis_started", "complementation_requested", "complementation_provided", "sent_to_presidency", "approved", "partially_approved", "presidency_rejected", "financial_classified", "returned", "procurement_completed"]).notNull(),
  note: text("note"),
